...
| R-003 | 2026-07-12 | Temporary firewall exception: outbound TCP/443 permitted from the Monitoring segment to support Splunk installation and package retrieval. While active, this exception weakens the SIEM egress restriction (C-001) by providing a general outbound HTTPS path that could be used for command-and-control or data exfiltration if the host were compromised. | Low | High | Medium | Mitigate | C-001 | Low (after closure) | Open (time-boxed) | A. LaSalle |
